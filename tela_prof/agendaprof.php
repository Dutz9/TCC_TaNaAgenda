<?php
    // =================================================================
    // BLOCO DE CONTROLE E DADOS - AGENDA DO PROFESSOR (v2 com Filtros)
    // =================================================================
    ...
    // Agora os filtros são arrays
    $filtros = [
        'periodo' => $_GET['periodo'] ?? [], // Espera um array
        'turma' => $_GET['turma'] ?? [],   // Espera um array
        'tipo' => $_GET['tipo'] ?? []     // Espera um array
    ];

    // 5. LÓGICA DE EVENTOS (COM FILTROS MÚLTIPLOS)
    ...
    // Filtra a lista de horários com base no filtro de período
    if (!empty($filtros['periodo'])) {
        $horarios_semana = [];
        if (in_array('Manha', $filtros['periodo'])) {
            $horarios_semana = array_merge($horarios_semana, ["07:10", "08:00", "08:50", "10:00", "10:50", "11:40"]);
        }
        if (in_array('Tarde', $filtros['periodo'])) {
            $horarios_semana = array_merge($horarios_semana, ["13:30", "14:20", "15:10", "16:20", "17:10", "18:00"]);
        }
        if (in_array('Noite', $filtros['periodo'])) {
            $horarios_semana = array_merge($horarios_semana, ["18:30", "19:20", "20:10", "21:20", "22:10"]);
        }
