document.addEventListener('DOMContentLoaded', () => {
    ...
    async function enviarDecisaoFinal(eventoId, decisao, botao) {
        ...
        try {
            ...
            if (response.ok && result.status === 'sucesso') {
                ...
                // --- LÓGICA DE ATUALIZAÇÃO DO CARD (NÃO MAIS REMOÇÃO) ---
                const cardAtualizado = document.querySelector(`.notificacao .detalhes-btn[data-id="${eventoId}"]`).closest('.notificacao');
                
                if (cardAtualizado) {
                    // 1. Encontra e atualiza o texto e a cor do status no card
                    const pStatus = cardAtualizado.querySelector('p[class*="status-"]');
                    const statusClasse = `status-${decisao.toLowerCase()}`; // ex: 'status-aprovado'
                    if (pStatus) {
                    ...
                    const btnDetalhes = cardAtualizado.querySelector('.detalhes-btn');
                    if (btnDetalhes) {
                        btnDetalhes.textContent = 'Ver Detalhes';
                    }
                }

                // 3. Atualiza a "memória" do JavaScript para o pop-up
    ...
    function abrirModalDecisao(evento) {
        ...
        if (evento.respostas_professores) {
            try {
                ...
                console.warn("JSON das respostas inválido:", evento.respostas_professores);
            }
        }
    ...
    /**
     * Mostra uma barra de feedback no topo da tela.
     */
