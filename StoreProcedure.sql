...
-- Procedure para listar todos os professores (para seleção manual)
DROP PROCEDURE IF EXISTS listarTodosProfessores$$
CREATE PROCEDURE listarTodosProfessores()
BEGIN
    SELECT 
        cd_usuario, 
        nm_usuario
    FROM 
        usuarios
    WHERE 
        tipo_usuario_ic_usuario = 'Professor'
    ORDER BY 
        nm_usuario;
END$$

-- Procedure para listar todos os coordenadores (para notificação)
DROP PROCEDURE IF EXISTS listarTodosCoordenadores$$
CREATE PROCEDURE listarTodosCoordenadores()
BEGIN
    SELECT 
        cd_usuario, 
        nm_usuario
    FROM 
        usuarios
    WHERE 
        tipo_usuario_ic_usuario = 'Coordenador'
    ORDER BY 
        nm_usuario;
END$$

-- Procedure para associar professores a um evento
DROP PROCEDURE IF EXISTS associarProfessoresAEvento$$
CREATE PROCEDURE associarProfessoresAEvento(
    IN pCdEvento VARCHAR(25),
    IN pCdUsuario VARCHAR(10)
)
BEGIN
    INSERT IGNORE INTO eventos_has_professores (eventos_cd_evento, usuarios_cd_usuario)
    VALUES (pCdEvento, pCdUsuario);
END$$

-- Procedure para notificar coordenadores sobre um evento (para 'resolucao_eventos_usuarios')
DROP PROCEDURE IF EXISTS notificarCoordenadoresSobreEvento$$
CREATE PROCEDURE notificarCoordenadoresSobreEvento(
    IN pCdEvento VARCHAR(25),
    IN pCdUsuarioCoordenador VARCHAR(10)
)
BEGIN
    -- Insere na tabela de resolução, mas com status 'Aprovado' ou 'Recusado' se o coordenador já tomou a decisão, 
    -- ou 'Pendente' se for para outro coordenador visualizar.
    -- Para eventos criados por coordenadores, o status inicial será 'Aprovado', então ele só precisa da entrada aqui.
    INSERT INTO resolucao_eventos_usuarios (eventos_cd_evento, usuarios_cd_usuario, status_resolucao)
    VALUES (pCdEvento, pCdUsuarioCoordenador, 'Aprovado') -- Coordenador já aprova ao criar
    ON DUPLICATE KEY UPDATE status_resolucao = 'Aprovado'; -- Em caso de duplicata, mantém aprovado.
END$$
